...
import { WASTE_TYPES, getMonthlyDepositStats } from "../../../utils/vikorAlgorithm";
import { utils, writeFile } from "xlsx";

/**
 * Organism: User — ReportTab
 * Menampilkan laporan bulanan setoran pribadi nasabah yang sedang login.
 * Fitur: expand row (mobile), export ke Excel.
 */
function ReportTab({ deposits }) {
  const [expandedRows, setExpandedRows] = useState(new Set());

  // Hitung statistik bulanan dari setoran nasabah ini saja
  const monthlyStats = getMonthlyDepositStats(deposits);
  ...
  const exportToExcel = () => {
    ...
    utils.book_append_sheet(workbook, worksheet, "Laporan Bulanan Saya");
    writeFile(workbook, "Laporan_Saya_BankSampah.xlsx");
  };
  ...
  return (
    <div className="fade-in">
      ...
      >
        <div>
          <h2 style={{ marginBottom: "var(--spacing-sm)" }}>Laporan Bulanan Saya</h2>
          <p style={{ color: "var(--text-secondary)", margin: 0, fontSize: "0.875rem" }}>
            Rekap setoran sampah Anda per bulan
          </p>
        </div>
        <button
          ...
          style={{ opacity: monthlyStats.length === 0 ? 0.5 : 1 }}
        >
          <Printer size={20} />
          Export Excel
        </button>
      ...
      <div className="card">
        <div className="table-container">
          <table className="table table-responsive-layout">
            <thead>
              <tr>
                <th style={{ width: "50px", padding: "10px 6px" }} className="mobile-only" />
              ...
              {monthlyStats.map((stat) => (
                <Fragment key={stat.month}>
                  <tr>
                    ...
                    >
                      Rp{" "}
                      {Number(stat.totalAmount).toLocaleString("id-ID", {
                        minimumFractionDigits: 0,
                        maximumFractionDigits: 0,
                      })}
                    </td>

                    {/* Detail jenis sampah — desktop only */}
                    <td className="desktop-only" style={{ padding: "10px 12px", textAlign: "left" }}>
                              ...
                              return (
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    <span style={{ fontSize: "0.65rem" }}> kg/L</span>
                                  </span>
                                  <span
                                    className="mobile-compact-text"
                                    style={{
                ...
                <tr>
                  <td colSpan="5" style={{ textAlign: "center", padding: "3rem", color: "var(--text-tertiary)" }}>
                    ...
                    <div style={{ fontSize: "0.875rem" }}>
                      Laporan akan muncul setelah Anda memiliki riwayat setoran.
                    </div>
                  </td>
                </tr>
              )}
